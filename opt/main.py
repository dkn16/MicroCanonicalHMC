...
def sample(m, sampling_alg, L, n):
    ...
    # add block to ensure that the computation is completed before saving the results
    b.block_until_ready()
    eevpd.block_until_ready()

    try:
        os.system('mkdir -p /pscratch/sd/d/dkn16/MicroCanonicalHMC/opt/data/'+ m.model.name + '/')
    except:
        pass
    np.savez('/pscratch/sd/d/dkn16/MicroCanonicalHMC/opt/data/'+ m.model.name + '/' + sampling_alg.name + str(L) +'_'+str(m.model.stochastic_grad)+ '.npz', stepsize= step_size, bias= b, eevpd= eevpd)
...
num_burnin = 10**4
models = [Model(StandardNormal( 100), [2.5, 20.], 0), # [2.5, 14.]
          Model(IllConditionedGaussian( 100,  1000), [0.4, 7.], num_burnin),
          ...
          Model(StochasticStandardNormal( 100,  1e-1), [2.5, 20.], 0),
          ]


if __name__ == '__main__':
    ialg, stochastic_grad, L, step_power = int(sys.argv[1]), float(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4])
    num_steps = 10**step_power
    model1 = Model(StochasticStandardNormal( 100,stochastic_grad), [2.5, 20.], 0)
    sample(model1, sampling_algs[ialg], L, num_steps)
